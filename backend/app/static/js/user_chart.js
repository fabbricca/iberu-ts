		...
		connectedCallback() {
			...
			this.chartElement.baselines[0].applyOptions({priceScaleId: 'right',
 			...
 			if (localStorage.getItem('show-balance') != 'true')
			 	document.querySelector('#user-balance').innerHTML = `<h1>******* ≈ ****</h1> 
			 																											 <span class="secondary-info">**********************************</span>`;
			else 
				document.querySelector('#user-balance').innerHTML = `<h1>${(RATE_OF_RETURN).toFixed(5)}% ≈ ${(RATE_OF_RETURN).toFixed(2)}$</h1> 
